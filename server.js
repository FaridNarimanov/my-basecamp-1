...
app.post('/users', async (req, res) => {
    ...
    try {
        ...
        db.run("INSERT INTO users (name, email, username, password) VALUES (?, ?, ?, ?)",
            [name, email, username, hashedPassword], function(err) {
...
app.post('/projects', requireLogin, (req, res) => {
    ...
    db.run("INSERT INTO projects (name, description, user_id) VALUES (?, ?, ?)",
        [name, description, req.session.userId], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
    ...
    db.get("SELECT id FROM users WHERE username = ?", [username], (err, user) => {
        ...
        `, [req.session.userId, req.params.id], (err, accessInfo) => {
            ...
            db.run("INSERT OR REPLACE INTO project_members (project_id, user_id, role) VALUES (?, ?, ?)",
                ...
                if (err) return res.status(500).json({ message: "Database error" });
                res.json({ message: `Member (@${username}) successfully added as ${role}!` });
            });
        });
    });
});
...
app.post('/projects/:id/discussions', requireLogin, requireProjectAccess, (req, res) => {
    ...
    db.run("INSERT INTO discussions (project_id, user_id, content) VALUES (?, ?, ?)",
        [req.params.id, req.session.userId, content], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
...
app.post('/projects/:id/tasks', requireLogin, requireProjectAccess, (req, res) => {
    ...
    db.run("INSERT INTO tasks (project_id, content) VALUES (?, ?)",
        [req.params.id, content], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
        res.status(201).json({ message: "Task added", id: this.lastID });
    });
});

app.patch('/tasks/:taskId', requireLogin, (req, res) => {
    const { is_completed } = req.body;
    db.run("UPDATE tasks SET is_completed = ? WHERE id = ?",
        [is_completed ? 1 : 0, req.params.taskId], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
...
app.delete('/projects/:id/attachments/:attachmentId', requireLogin, requireProjectAccess, (req, res) => {
    db.get(`
        SELECT p.user_id as owner_id, pm.role 
        FROM projects p 
        LEFT JOIN project_members pm ON p.id = pm.project_id AND pm.user_id = ? 
        WHERE p.id = ?
    `, [req.session.userId, req.params.id], (err, accessInfo) => {
        if (err || !accessInfo) return res.status(404).json({ message: "Not found" });
        ...
        db.get("SELECT file_path FROM attachments WHERE id = ? AND project_id = ?", [req.params.attachmentId, req.params.id], (err, file) => {
            if (file) {
                const fullPath = path.join(__dirname, 'public', file.file_path);
                if (fs.existsSync(fullPath)) fs.unlinkSync(fullPath);
            }
            db.run("DELETE FROM attachments WHERE id = ? AND project_id = ?", [req.params.attachmentId, req.params.id], function(err) {
                if (err) return res.status(500).json({ message: "Database error" });
...
app.post('/projects/:id/attachments', requireLogin, requireProjectAccess, upload.single('file'), (req, res) => {
    if (!req.file) return res.status(400).json({ message: "No file uploaded" });
    const filePath = '/uploads/' + req.file.filename;
    db.run("INSERT INTO attachments (project_id, file_name, file_path, uploaded_by) VALUES (?, ?, ?, ?)",
        [req.params.id, req.file.originalname, filePath, req.session.userId], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
...
app.put('/profile', requireLogin, async (req, res) => {
    ...
    try {
        db.get("SELECT id FROM users WHERE username = ? AND id != ?", [username, req.session.userId], async (err, existing) => {
            ...
            if (newPassword && newPassword.trim() !== '') {
                db.get("SELECT password FROM users WHERE id = ?", [req.session.userId], async (err, user) => {
                    ...
                    db.run("UPDATE users SET name=?, email=?, username=?, password=? WHERE id=?",
                        [name, email, username, hashed, req.session.userId], function(err) {
                        if (err) return res.status(500).json({ message: "Database error" });
                ...
                db.run("UPDATE users SET name=?, email=?, username=? WHERE id=?",
                    [name, email, username, req.session.userId], function(err) {
                    if (err) return res.status(500).json({ message: "Database error" });
...
app.post('/profile/picture', requireLogin, upload.single('avatar'), (req, res) => {
    if (!req.file) return res.status(400).json({ message: "No file uploaded" });
    const filePath = '/uploads/' + req.file.filename;
    db.run("UPDATE users SET profile_pic = ? WHERE id = ?", [filePath, req.session.userId], function(err) {
        if (err) return res.status(500).json({ message: "Database error" });
